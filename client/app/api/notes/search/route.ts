...
export async function GET(request: NextRequest) {
  try {
    ...
    // Build search query - Convert userId string to ObjectId for proper matching
    ...
    console.error('Search Notes API Route Error:', error)
    if (error instanceof Error && error.message.includes('Authentication')) {
      return createErrorResponse('Authentication required. Please log in again.', 401)
    }
    return handleApiError(error, 'Search notes')
  }
}
