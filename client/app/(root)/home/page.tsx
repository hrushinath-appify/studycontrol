        ...
        <div className="bg-gradient-to-r from-blue-50/80 to-purple-50/80 dark:from-blue-950/30 dark:to-purple-950/30 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-blue-200/50 dark:border-blue-800/30 backdrop-blur-sm">
          <div className="text-center space-y-3 sm:space-y-4">
            <div className="flex justify-center items-center gap-2 sm:gap-3">
              <span className="text-xl sm:text-2xl md:text-3xl lg:text-4xl flex items-center">💡</span>
              <button
                onClick={loadQuote}
                disabled={isLoading}
                className="p-2 sm:p-2.5 md:p-3 rounded-full bg-white/50 dark:bg-black/20 hover:bg-white/70 dark:hover:bg-black/40 transition-all duration-200 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation active:scale-95 flex items-center justify-center"
                title="Get new inspirational quote from curated collection"
              >
                <RefreshCw className={`w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 text-blue-600 dark:text-blue-400 ${isLoading ? 'animate-spin' : ''}`} />
              </button>
            </div>
            {isLoading ? (
